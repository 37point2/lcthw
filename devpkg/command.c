...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only) {
	...
	if(apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
		...
		check(rc == APR_SUCCESS, "Failed to untar %s", TAR_GZ_SRC);
		...
		rc = apr_dir_make_recursive(BUILD_DIR, APR_UREAD | APR_UWRITE | APR_UEXECUTE, p);
...
int Command_build(apr_pool_t *p, const char *url, const char *configur_opts, 
	...
	if(access(CONFIG_SCRIPT, X_OK) == 0) {
		...
		rc = Shell_exec(CONFIGURE_SH, "OPTS", configur_opts, NULL);
...
int Command_install(apr_pool_t *p, const char *url, const char *configur_opts,
	const char *make_opts, const char *install_opts) {
	int rc = 0;
	...
	if(rc == 1) {
		rc = Command_build(p, url, configur_opts, make_opts, install_opts);
